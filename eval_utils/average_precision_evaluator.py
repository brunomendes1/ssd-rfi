...
from data_generator.object_detection_2d_photometric_ops import ConvertTo3Channels # problematic because my images have only one channel
...
class Evaluator:
    ...
    def __init__(self,
        ...
        '''
        WARNING: filenames can only have a dot before the file extension
        Arguments:
    ...
    def __call__(self,
        ...
        #############################################################################################
	#BM 3/24/2025 assigned output of function to variable preds
        #preds = 
        ...
        #############################################################################################
	#BM 3/24/2025 assigned output of function to variable prec_rec
        prec_rec = self.compute_precision_recall(verbose=verbose, ret=True) # BM 3/24/2025
...
#        print(prec_rec)
        ...
        avg_precision = self.compute_average_precisions(mode=average_precision_mode,
                                        num_recall_points=num_recall_points,
                                        verbose=verbose,
                                        ret=True) # BM 3/24/2025
...
#                                                          'inverse_transform',
        ...
        for j in tr:
            ...
            batch_X, batch_image_ids, batch_eval_neutral, batch_orig_labels = next(generator)
            #print(batch_image_ids, batch_eval_neutral, batch_orig_labels)
            # Predict.
