...
class DataGenerator:
            ...
            for row in csvread: # For every line (i.e for every bounding box) in the CSV file...
                if self.include_classes == 'all' or int(row[self.input_format.index('class_id')].strip()) in self.include_classes: # If the class_id is among the classes that are to be included in the dataset...
                    ...
                    for element in self.labels_output_format: # For each element in the output format (where the elements are the class ID and the four box coordinates)...
                        # BM 10/21/2024print("row ", row, "box: ", box, "\n")
        ...
        if self.load_images_into_memory:
            ...
            #BM 10/4/24
                ...
                with open(filename, 'r') as file:
                    ...
                    # Convert the list of lists into a NumPy array with uint8 dtype
                    # BM 2/20/25 image_array = image_array.astype(np.uint8)
                    # BM 10/24/24 We are dealing with grayscale images
                    image_array = image_array.astype(np.float32)
                    # A user of Ferrari's github page proposes the following change:
                    image_array  = np.reshape (image_array, image_array.shape+(1,))
                    #print(image_array.ndim)
                    self.images.append(image_array)
                # BM 10/23/2024
                #print("image dimension: ", image_array)
    ...
    def create_hdf5_dataset(self,
        ...
        for i in tr:
            ...
            with open(self.filenames[i], 'r') as file:
                reader = csv.reader(file)
                  # Convert the CSV reader object to a list of lists
                ...
                #BM 10/23/24
                # Make sure all images end up having three channels. The code below will not work with data from csv files
                if image.ndim == 2:
